    ...
    <>
          ...
          <div className="flex-1 text-sm text-muted-foreground">
            {selectedCount > 0 ? (
              <span className="font-medium text-primary">
                {selectedCount} trong số {totalRows} tuyến xe được chọn
              </span>
            ) : (
              <span>Tổng số {totalRows} tuyến xe</span>
            )}
          </div>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {confirmAction.type === "lock" && "Xác nhận vô hiệu hóa"}
              {confirmAction.type === "unlock" && "Xác nhận kích hoạt"}
              {confirmAction.type === "export" && "Xác nhận xuất Excel"}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {confirmAction.type === "lock" &&
                `Bạn có chắc chắn muốn vô hiệu hóa ${selectedCount} tuyến xe đã chọn?`}
              {confirmAction.type === "unlock" &&
                `Bạn có chắc chắn muốn kích hoạt ${selectedCount} tuyến xe đã chọn?`}
              {confirmAction.type === "export" &&
                `Bạn có muốn xuất ${selectedCount} tuyến xe đã chọn ra file Excel?`}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
