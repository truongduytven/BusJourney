...
export function DataTablePagination({
  ...
  const handleConfirmAction = () => {
    ...
    if (confirmAction.type === "lock") {
      onBulkToggleActive(false, selectedIds);
    } else if (confirmAction.type === "unlock") {
      onBulkToggleActive(true, selectedIds);
          ...
          <div className="flex-1 text-sm text-muted-foreground">
            {selectedCount > 0 ? (
              <span className="font-medium text-primary">
                {selectedCount} trong số {totalRows} template được chọn
              </span>
            ) : (
              <span>Tổng số {totalRows} template</span>
            )}
          </div>
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {confirmAction.type === "lock" &&
                `Bạn có chắc chắn muốn tạm dừng ${selectedCount} template đã chọn?`}
              {confirmAction.type === "unlock" &&
                `Bạn có chắc chắn muốn kích hoạt ${selectedCount} template đã chọn?`}
              {confirmAction.type === "export" &&
                `Bạn có muốn xuất ${selectedCount} template đã chọn ra file Excel?`}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
