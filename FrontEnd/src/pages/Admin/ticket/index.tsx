...
export default function AdminTicketsPage() {
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      if (confirmDialog.type === "toggle") {
        await dispatch(toggleAdminTicketStatus(confirmDialog.ticketId)).unwrap();
        toast.success("Đã cập nhật trạng thái vé");
      } else if (confirmDialog.type === "delete") {
        await dispatch(deleteAdminTicket(confirmDialog.ticketId)).unwrap();
  ...
  const handleModalSubmit = async (data: {
    ...
    try {
      await dispatch(updateAdminTicket({
  ...
  return (
    <div className="container mx-auto py-6">
      <div className="mb-6">
        ...
        <p className="text-gray-500">Quản lý tất cả vé trong hệ thống</p>
      </div>
