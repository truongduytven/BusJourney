import { SlidersHorizontal, Star, Eye, X } from "lucide-react";
...
export function DataTableAdvancedFilters({
  ...
  const hasActiveFilters = rating !== "all" || isVisible !== "all";
        ...
        <Button variant="outline" size="sm" className="h-8 gap-2 relative border-gray-400 cursor-pointer">
          ...
          {hasActiveFilters && (
            <span className="absolute -top-1 -right-1 flex h-4 w-4 items-center justify-center rounded-full bg-primary text-[10px] font-bold text-white">
              {(rating !== "all" ? 1 : 0) + (isVisible !== "all" ? 1 : 0)}
            </span>
          )}
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end" className="w-[280px]">
        ...
        <div className="space-y-4 p-4">
          ...
          <div className="space-y-2">
            ...
            <Select value={isVisible} onValueChange={onVisibilityChange}>
              <SelectTrigger className="bg-orange-50 border-orange-200 hover:bg-orange-100 transition-colors">
