...
interface DataTableProps {
  columns: ColumnDef<TypeBus>[];
  data: TypeBus[];
  totalRows: number;
  searchQuery: string;
  isFloors: string;
  onSearchChange: (search: string) => void;
  onIsFloorsChange: (isFloors: string) => void;
  onPageSizeChange: (pageSize: number) => void;
  onResetFilters: () => void;
  isReadOnly?: boolean; // For Admin read-only mode
  onCreateTypeBus?: () => void;
      ...
      <div className="flex flex-wrap items-center gap-2 py-5">
        ...
        <div className="ml-auto flex gap-x-4">
          <DataTableAdvancedFilters
            isFloors={isFloors}
            onIsFloorsChange={onIsFloorsChange}
            onReset={onResetFilters}
          />
          {!isReadOnly && onCreateTypeBus && (
            <Button onClick={onCreateTypeBus} size="sm" className="h-8 gap-2">
