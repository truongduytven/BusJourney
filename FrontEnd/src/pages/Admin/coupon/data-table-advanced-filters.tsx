import { SlidersHorizontal, Activity, X, Percent, DollarSign } from "lucide-react";
...
export function DataTableAdvancedFilters({
  ...
  const hasActiveFilters = status !== "all" || discountType !== "all";
        ...
        <Button variant="outline" size="sm" className="h-8 gap-2 relative border-gray-400 cursor-pointer">
          ...
          {hasActiveFilters && (
            <span className="absolute -top-1 -right-1 flex h-4 w-4 items-center justify-center rounded-full bg-primary text-[10px] font-bold text-white">
              {(status !== "all" ? 1 : 0) + (discountType !== "all" ? 1 : 0)}
            </span>
          )}
        </Button>
      </DropdownMenuTrigger>
