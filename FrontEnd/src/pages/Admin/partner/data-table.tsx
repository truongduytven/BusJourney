...
interface DataTableProps {
  ...
  onResetFilters: () => void;
  onBulkToggleStatus: (partnerIds: string[], targetStatus: string) => void;
}
...
export function DataTable({
  ...
  const selectedPartners = table.getFilteredSelectedRowModel().rows.map(row => row.original);

  const handleBulkMarkProcessed = () => {
    const partnerIds = selectedPartners.map(p => p.id);
    onBulkToggleStatus(partnerIds, "processed");
    table.resetRowSelection();
  };

  const handleBulkMarkUnprocessed = () => {
    const partnerIds = selectedPartners.map(p => p.id);
    onBulkToggleStatus(partnerIds, "unprocessed");
    table.resetRowSelection();
  };
    ...
    <div>
      ...
      <div className="py-4">
        <DataTablePagination
          ...
          onBulkMarkUnprocessed={handleBulkMarkUnprocessed}
        />
      </div>
    </div>
  );
